...
public class SuperstitionSerializableModel implements Serializable {
	...
	public void initializeMonsters() {
		...
		for(int i = 0; i < 10_000; i++) {
			...
			if(rnd < 30) {
				data = new MonsterData(SuperstitionGameElements.MONSTRUM_ANIMATION, SuperstitionGameElements.WOODGOLEM_DIE_ANIMATION);
			} else if(rnd < 60) {
				data = new MonsterData(SuperstitionGameElements.MONKEY_ANIMATION, SuperstitionGameElements.WOODGOLEM_DIE_ANIMATION);
			} else {
				data = new MonsterData(SuperstitionGameElements.RUNNING_ANIMATION, SuperstitionGameElements.WOODGOLEM_DIE_ANIMATION);
			}
		...
		// Behemoth.
		MonsterData behemoth = new MonsterData(SuperstitionGameElements.BEHEMOTH_ANIMATION, SuperstitionGameElements.WOODGOLEM_DIE_ANIMATION, 1_000_000.0);
